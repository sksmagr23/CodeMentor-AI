  ...
  return (
    <div className="rounded-xl border border-red-900/60 bg-slate-900/90 p-5 shadow-lg my-3 text-slate-200">
      <div className="flex items-center gap-2.5 border-b border-slate-800/80 pb-3 mb-4">
        ...
        <div>
          <h3 className="font-['Space_Grotesk'] text-base font-semibold text-red-200">
            Bug & Failure Analysis
          </h3>
          <span className="text-xs text-red-400/90">{renderSafeText(issue)}</span>
        </div>
      </div>
      ...
      <div className="space-y-3.5 text-xs sm:text-sm">
        <div className="p-3 rounded-lg bg-slate-950/60 border border-slate-800">
          ...
          <p className="text-slate-300 leading-relaxed">{renderSafeText(why_it_fails)}</p>
          {failing_condition && (
        ...
        <div>
          ...
          <div className="p-2.5 rounded-lg bg-slate-950 border border-slate-800 font-['JetBrains_Mono'] text-xs text-amber-300 mb-2">
            Input: {renderSafeText(counterexample)}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-['JetBrains_Mono']">
            <div className="p-2 rounded bg-red-950/30 border border-red-500/20 text-red-300">
              <span className="text-red-400 font-sans block text-[11px]">Actual Code Output:</span>
        ...
        <div className="p-3 rounded-lg bg-emerald-950/20 border border-emerald-500/30">
          ...
          <p className="text-slate-300 text-xs leading-relaxed">{renderSafeText(fix)}</p>
        </div>
